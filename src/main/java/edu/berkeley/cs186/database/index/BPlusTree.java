...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> backNode = root.put(key, rid);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        if (scanAll().hasNext()) {
            throw new BPlusTreeException("bulkLoad must be called in an empty tree");
        }
        while (data.hasNext()) {
            // root will be change
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode node, DataBox key) {
            this.currNode = node;
            this.currIter = node.scanGreaterEqual(key);
        }
